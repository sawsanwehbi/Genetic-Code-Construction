...
# Following dataframe has the input needed for plotting figures
AA_properties_plots <- read.csv( 'AA_properties_Fig1_SupFig1.csv')
...
## 
NodivLUCAclanratio_se <- NodivLUCAclanratio_se[match(AA_properties_plots$AA,names(NodivLUCAclanratio_se))]
NoDiv_LUCAusage <- NoDiv_LUCAusage[match(  AA_properties_plots$AA, names(NoDiv_LUCAusage))]
stopifnot(all.equal(as.numeric(NoDiv_LUCAusage), AA_properties_plots$LUCA_usage))
...
NoDivLUCA_NSA_plot <- ggplot(AA_properties_plots, aes(y = as.numeric(LUCA_usage), 
  ...
  annotate(geom="text", x=0, y=1.23, label="a)", color="black", size = 16) +
...
class1r2 <- summary(ModifiedNoDivNSA_modelclass1)$r.squared
class1pv <- summary(ModifiedNoDivNSA_modelclass1)$coefficients[2, 4]
class2r2 <- summary(ModifiedNoDivNSA_modelclass2)$r.squared
class2pv <- summary(ModifiedNoDivNSA_modelclass2)$coefficients[2, 4]
...
ModifiedNoDivLUCA_NSA_plot <- ggplot(BiosynReqmodified, aes(y = as.numeric(LUCA_usage), 
  ...
  theme(axis.text=element_text(size=24),axis.title=element_text(size=32,face="bold"),
        legend.text=element_text(size=14), legend.title = element_text(size=16,face="bold")) + 
  annotate(geom="text", x=0, y=1.23, label="b)", color="black", size = 16) +
  ...
  annotate(geom="text", x=7, y=1.2, label=paste0(paste0('Weighted R',supsc('2')), "= 0.69"), color= "#F8766D", size = 14) +
  annotate(geom="text", x=7, y=1.15, label="p = 0.002", color= "#F8766D", size = 14) +
...
grid.arrange(NoDivLUCA_NSA_plot, ModifiedNoDivLUCA_NSA_plot, ncol=2)
